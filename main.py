...
async def visit(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if used >= limit:
        await update.message.reply_text("❌ Group daily visit limit reached!")
        return

    args = context.args
    if len(args) != 2:
        await update.message.reply_text("⚠️ Usage: /visit <region> <uid>")
        return
    ...
    if not is_vip:
        user_info = user_data.get(user_id, {})
        if user_info.get("date") == today and user_info.get("count", 0) >= 1:
            await processing_msg.edit_text("⛔ You have used your free visit today.")
            return
        user_data[user_id] = {"date": today, "count": user_info.get("count", 0)}

    try:
        url = f"https://ffvisit-0.vercel.app/{region}/{uid}?key=SPIDEYxVISIT"
        response = requests.get(url)
        data = response.json()
        logger.info(f"Visit API response: {data}")
    except Exception as e:
        logger.error(f"Visit API error: {e}")
        await processing_msg.edit_text("🚨 API Error! Try again later.")
        return

    # Remove 'key' from response if present so it won't be shown
    if isinstance(data, dict):
        data.pop("key", None)

    if not isinstance(data, dict) or not all(k in data for k in ["nickname", "uid", "level", "region", "success"]):
        await processing_msg.edit_text("⚠️ Invalid UID or unable to fetch details.🙁 Please check UID or try again later.")
        logger.warning(f"Incomplete visit API response for UID {uid}: {data}")
        return
    ...
    # Build text using format() to avoid f-string multiline issues
    text = ("✅ Visit Done Successfully!\n\n"
            "👤 Name: {}\n"
            "🆔 UID: {}\n"
            "📊 Level: {}\n"
            "🌍 Region: {}\n"
    ...
    try:
        ...
        logger.error(f"Error handling visit photo: {e}")
        await processing_msg.edit_text(text)
